...
public class ManagerController {
    ...
    @GetMapping("/books")
    public String books(@RequestParam(required = false) String q, @RequestParam(required = false) Long categoryId,
            @RequestParam(required = false) String status, @RequestParam(required = false) BigDecimal minPrice,
            @RequestParam(required = false) BigDecimal maxPrice, @RequestParam(required = false) Integer minStock,
            @RequestParam(defaultValue = "title") String sort, @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size, Model model) {
        model.addAttribute("pageResult",
                catalog.searchBooks(q, categoryId, status, minPrice, maxPrice, minStock, sort, page, size));
    ...
    public String delete(@PathVariable Long id, Authentication auth) {
        catalog.softDeleteBook(id);
        audit.log(current.current(auth).getId(), "BOOK", id, "SOFT_DELETE", null, "INACTIVE");
        return "redirect:/manager/books";
    }
    ...
    public String authors(Model model) {
        model.addAttribute("authors", catalog.authors());
    ...
    public String coupons(Model model) {
        model.addAttribute("coupons", catalog.coupons());
        model.addAttribute("coupon", new Coupon());
        return "coupons";
    }

    @PostMapping("/coupons")
    public String saveCoupon(@RequestParam String code, @RequestParam String discountType,
            @RequestParam BigDecimal discountValue, @RequestParam BigDecimal minOrderAmount,
        ...
        c.setEndsAt(endsAt);
        c.setStatus(status);
        catalog.saveCoupon(c);
        return "redirect:/manager/coupons";
    }

    @GetMapping("/orders")
    public String allOrders(Model model) {
        model.addAttribute("orders", orders.findOrders(null, true));
        return "manager-orders";
    }

    @PostMapping("/orders/{id}/status")
    public String status(@PathVariable Long id, @RequestParam String status, @RequestParam String paymentStatus,
            @RequestParam String shipmentStatus) {
        orders.updateStatus(id, status, paymentStatus, shipmentStatus);
    ...
    public String resStatus(@PathVariable Long id, @RequestParam String status) {
    ...
    public ResponseEntity<String> exportBooks() {
        var page = catalog.searchBooks(null, null, null, null, null, null, "title", 0, 100);
        StringBuilder sb = new StringBuilder("sku,title,category,price,stock,status\n");
        page.getItems()
                .forEach(b -> sb.append(b.getSku()).append(',').append('"').append(b.getTitle()).append('"').append(',')
                        .append(b.getCategoryName()).append(',').append(b.getListPrice()).append(',')
